...
- **`mine=false`(둘러보기)가 방금 비공개→전체공개로 바꾼 본인 기록을 안 보여주는 버그 확인됨**: `GET /api/records/{id}` 상세 조회로는 `visibility: "PUBLIC"`이 정확히 반영된 게 확인되는데(=PATCH·프론트는 정상), 그 직후 `GET /api/records?mine=false`엔 그 recordId가 안 나타남. 프론트에서 우회할 방법 없음 — 백엔드 쪽 목록 API 문제로 보고함(캐싱 지연인지 필터 로직 버그인지는 미확인), 미해결.
